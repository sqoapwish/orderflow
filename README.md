...
> Текущий статус: завершены инженерная основа, аутентификация, каталог и складской учёт. Следующий бизнес-этап — корзина и транзакционное оформление заказа. Возможности не отмечаются готовыми до реализации и тестирования.
...
Integration-тесты проверяют инфраструктуру, полный auth-flow, каталог и конкурентные складские резервы на настоящем PostgreSQL. В CI PostgreSQL, Redis и RabbitMQ поднимаются автоматически.
